...
class TitleAnimator extends React.Component {
  animatedValues = [];
  animatedColorValues = [];
  animations = [];
  ...
  animate(toValue = 1) {
    ...
    this.animations = this.textArr.map((_, i) => {
      ...
      const colorChangeAnimation = Animated.timing(
        ...
        {
          toValue: i >= this.textArr.length - 2 ? toValue : 0, // Change color only for the last two elements
          duration: this.props.timing,
          useNativeDriver: false,
        }
      );
      // Fade-out animation for words not supposed to be blue
      const fadeOutAnimation = Animated.timing(this.animatedValues[i], {
        toValue: i < this.textArr.length - 2 ? 0 : toValue,
        duration: this.props.timing,
        useNativeDriver: false,
      });
